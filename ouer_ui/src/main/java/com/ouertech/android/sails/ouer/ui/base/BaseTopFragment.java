 ...
 * desc   : 带标题栏的界面
 */
public abstract class BaseTopFragment extends BaseFullFragment {
    //工具栏（标题栏）
    ...
    protected View initBaseViews() {
        View view = super.initBaseViews();

        ViewStub topStub = (ViewStub)view.findViewById(R.id.ouer_id_stub_top);
        topStub.inflate();

        mTopbar = view.findViewById(R.id.ouer_id_top);

        mTopbarShdow = view.findViewById(R.id.ouer_id_shadow);
        mTopbarShdow.setVisibility(View.VISIBLE);

        mTvTitle = (TextView)view.findViewById(R.id.ouer_id_title);
        mIbNavigation = (ImageButton)view.findViewById(R.id.ouer_id_navigation);
