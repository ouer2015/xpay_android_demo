...
public class PaymentsAdapter extends BaseAdapter{
    ...
    public PaymentsAdapter(Activity activity, List<Payment> payments) {
    ...
    @Override
    public int getCount() {
        return UtilList.getCount(mPayments);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Button btnPayment = new Button(mActivity);
        btnPayment.setText(payment.getName());
        btnPayment.setOnClickListener(new View.OnClickListener(){

            @Override
            public void onClick(View v) {
                order(payment.getChannel(), "测试的商品", "该测试商品的详细描述", "0.01");
            }
        });
    ...
    private void order(String channel, String subject, String body, String price) {
        if(CstXPay.CHANNEL_WX.equals(channel)) {
            new HttpFuture.Builder(mActivity, CstHttp.POST)
                    ...
                    .setListener(new OuerFutureListener(mActivity) {
                        @Override
                        public void onComplete(AgnettyResult result) {
                            super.onComplete(result);
                            String charge = (String) result.getAttach();
                            XPay.pay(mActivity, charge);
                        }
                    })
                    .execute();
